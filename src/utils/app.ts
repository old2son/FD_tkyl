...
function checkSign(uid: any, sign: any, _random_: any, callback: Function) {
	Base.ajax({
		url: Base.api.art.domain + '/api/CheckSign',
		data: {
			uid: uid,
			sign: sign,
			_random_: _random_
		},
		success: function (res: { Data: { uid: string; sign: string } }) {
			sessionStorage.setItem('ownuid', res.Data.uid);
			sessionStorage.setItem('ownsign', res.Data.sign);
		},
		complete: callback,
		error: function (e: any) {
			// Base.checkSign(0, '', '', callback);
			console.log('验证失败');
		}
	});
}
...
var Base = {
	...
	defaultShareImgUrl: 'https://tkyl-health.familydoctor.cn/static/img/logo_share.png',
	...
	throttle: function (func: Function, wait: number, options?: any) {
		var timeout: null | undefined | number | NodeJS.Timeout, context: null, args: IArguments | null, result: any;
		var previous = 0;
	...
	debounce: function (func: Function, wait: number, immediate: boolean) {
		var timeout: null | undefined | number | NodeJS.Timeout,
			context: null,
	...
	getArtToken: function (time: any) {
		return Base.md5(Base.api.tk + '23' + Base.md5(Base.md5(time)));
	},

	// 模拟滑动条
	fakeRange: function (eles: any) {
		...
		function init($ele: HTMLElement) {
			var $range = $ele.querySelector('.range-plugin') as HTMLElement;
			var $input = $ele.querySelector('input[type=text]') as HTMLInputElement;
			var $track = $range.querySelector('.uper') as HTMLElement;
			var $thumb = $range.querySelector('.thumb') as HTMLElement;
			var $val = $ele.querySelector('.js-val') as HTMLElement;
			var min = parseInt($range.getAttribute('data-min') as string);
			var max = parseInt($range.getAttribute('data-max') as string);
			var del = max - min;
			var curr = parseInt($range.getAttribute('data-val') as string) || 0;
			var pageWidth = window.innerWidth;
			var rangeWidth = $range.clientWidth;
			var sideOffset = (pageWidth - rangeWidth) / 2;
			...
			$range.addEventListener('mousedown', function (e: MouseEvent) {
				if (e.button !== 0) {
					return;
				}
			...
			$range.addEventListener('touchstart', function (e: TouchEvent) {
				drop(e);
			});

			$range.addEventListener('touchmove', function (e: TouchEvent) {
				move(e);
			});

			function drop(e: MouseEvent | TouchEvent) {
				...
				if (touches) {
					...
					currX = touches[0].pageX - sideOffset;
				} else {
					currX = (e as MouseEvent).pageX - sideOffset;
				}

				if (currX === undefined) {
					throw new Error('currX is undefined');
				}

				e.preventDefault();
				e.stopPropagation();

				if (currX >= 0 && currX <= rangeWidth) {
					var currVal = (currX / rangeWidth) * del + min;
					currVal = parseInt(currVal.toFixed(0));
					setPercent(currVal);
				}
			...
			function setPercent(val: any) {
				var per: any = ((val - min) / del) * 100;

				per = per.toFixed(0);
				$track.style.width = 100 - per + '%';
				$thumb.style.left = per + '%';
				$val.innerText = per + '%';
				$range.setAttribute('data-val', val);
				$input.value = val;
	...
	initGotoTop: function () {
		if (document.querySelector('.btn-goto-top')) {
			return;
		}
		...
		btn.addEventListener('click', function () {
			...
			timer = requestAnimationFrame(function func() {
				...
				document.documentElement.scrollTop = document.body.scrollTop = (t * -c) / duration + top;
